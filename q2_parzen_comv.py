...
from matrix import complete_view, lmatrix
...
results_parzen_comv = np.zeros((300, 1))
...
for n in range(30):
    ...
    for o in range(10):
        ...
        result_parzen_comv = accuracy_score(test_result[:,0], test_result[:,1])
        print("A acurácia da iteração", n+o+1, "foi de:", result_parzen_comv)

        for i in range(n, n + 1):
            for j in range(1):
                results_parzen_comv[i, j] = result_parzen_comv

        np.savetxt("q2_parzen_comv.txt", results_parzen_comv, fmt="%.6f")