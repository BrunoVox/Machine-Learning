...
for train_index, test_index in rskf.split(shape_view, lmatrix):
    ...
    for i in range(1890):
        if lmatrix[train_index[i]] == "GRASS":
            mean_vector_grass += shape_view[train_index[i]]
        elif lmatrix[train_index[i]] == "WINDOW":
            mean_vector_window += shape_view[train_index[i]]
        elif lmatrix[train_index[i]] == "CEMENT":
            mean_vector_cement += shape_view[train_index[i]]
        elif lmatrix[train_index[i]] == "BRICKFACE":
            mean_vector_brickface += shape_view[train_index[i]]
        elif lmatrix[train_index[i]] == "SKY":
            mean_vector_sky += shape_view[train_index[i]]
        elif lmatrix[train_index[i]] == "FOLIAGE":
            mean_vector_foliage += shape_view[train_index[i]]
        elif lmatrix[train_index[i]] == "PATH":
            mean_vector_path += shape_view[train_index[i]]    
    ...
    for i in range(1890):
        if lmatrix[train_index[i]] == "GRASS":
            covar_grass += np.dot(np.transpose(shape_view[train_index[i]] - mean_vector_grass), shape_view[train_index[i]] - mean_vector_grass)
        elif lmatrix[train_index[i]] == "WINDOW":
            covar_window += np.dot(np.transpose(shape_view[train_index[i]] - mean_vector_window), shape_view[train_index[i]] - mean_vector_window)
        elif lmatrix[train_index[i]] == "CEMENT":
            covar_cement += np.dot(np.transpose(shape_view[train_index[i]] - mean_vector_cement), shape_view[train_index[i]] - mean_vector_cement)
        elif lmatrix[train_index[i]] == "BRICKFACE":
            covar_brickface += np.dot(np.transpose(shape_view[train_index[i]] - mean_vector_brickface), shape_view[train_index[i]] - mean_vector_brickface)
        elif lmatrix[train_index[i]] == "SKY":
            covar_sky += np.dot(np.transpose(shape_view[train_index[i]] - mean_vector_sky), shape_view[train_index[i]] - mean_vector_sky)
        elif lmatrix[train_index[i]] == "FOLIAGE":
            covar_foliage += np.dot(np.transpose(shape_view[train_index[i]] - mean_vector_foliage), shape_view[train_index[i]] - mean_vector_foliage)
        elif lmatrix[train_index[i]] == "PATH":
            covar_path += np.dot(np.transpose(shape_view[train_index[i]] - mean_vector_path), shape_view[train_index[i]] - mean_vector_path)
    ...
    for i in range(210):
        px_grass = multivar_density(shape_view[test_index[i]], np.ravel(mean_vector_grass), covar_grass)
        px_window = multivar_density(shape_view[test_index[i]], np.ravel(mean_vector_window), covar_window)
        px_cement = multivar_density(shape_view[test_index[i]], np.ravel(mean_vector_cement), covar_cement)
        px_brickface = multivar_density(shape_view[test_index[i]], np.ravel(mean_vector_brickface), covar_brickface)
        px_sky = multivar_density(shape_view[test_index[i]], np.ravel(mean_vector_sky), covar_sky)
        px_foliage = multivar_density(shape_view[test_index[i]], np.ravel(mean_vector_foliage), covar_foliage)
        px_path = multivar_density(shape_view[test_index[i]], np.ravel(mean_vector_path), covar_path)
...
result_bay_shapev = correct / total