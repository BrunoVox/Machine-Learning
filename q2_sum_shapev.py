...
from q2_parzen_shapev import h_test_shapev
...
results_ensemble_shapev = np.zeros((300, 1))
...
for n in range(300):
    ...
    result_ensemble_shapev = accuracy_score(test_result[:,0], test_result[:,1])
    print("A acurácia da iteração", n+1, "foi de:", result_ensemble_shapev)

    for i in range(n, n + 1):
        for j in range(1):
            results_ensemble_shapev[i, j] = result_ensemble_shapev

    np.savetxt("q2_ensemble_shapev.txt", results_ensemble_shapev, fmt="%.6f")