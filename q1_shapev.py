from matrix import lmatrix, index_matrix, kcm_check, shape_view
import random
import numpy as np 
    ...
    for i in range(c):
        for k in range(len(clusters)):
            if clusters[k] == 0 and i == 0:
                num[i] += np.dot(kernel_gh(hyperparameter_vector, shape_view, prototypes_matrix, k, i), shape_view[k])
                den[i] += kernel_gh(hyperparameter_vector, shape_view, prototypes_matrix, k, i)
        ...
        for h in range(p):
            for i in range(c): 
                for k in range(len(clusters)):
                    if clusters[k] == 0 and i == 0:
                        if j == 0:
                            prod[h] += kernel_gh(hyperparameter_vector, shape_view, prototypes_matrix, k, i) * (shape_view[k, h] - prototypes_matrix[i, h]) ** 2
                        if h == 0:    
                            den2[j] += kernel_gh(hyperparameter_vector, shape_view, prototypes_matrix, k, i) * (shape_view[k, j] - prototypes_matrix[i, j]) ** 2
                    elif clusters[k] == 1 and i == 1:
                        if j == 0:
                            prod[h] += kernel_gh(hyperparameter_vector, shape_view, prototypes_matrix, k, i) * (shape_view[k, h] - prototypes_matrix[i, h]) ** 2
                        if h == 0:    
    ...
    for i in range(c):
        for k in range(len(clusters)):
            if i == 0 and clusters[k] == 0:
                dec += 2 * (1 - kernel_gh(hyperparameter_vector, shape_view, prototypes_matrix, k, i))
...
results_prototypes = np.zeros((700, 8))
results_objects = np.zeros((100, 2100))
results_hyperparameter = np.zeros((100, 8))
results_ari = np.zeros((100, 1))

euc_distance = euclidean_distances(shape_view) ** 2
...
for n in range(100):
    ...
    for i in range(len(prototypes_vector)):
        for j in range(p):
            prototypes_matrix[i, j] = shape_view[int(prototypes_vector[i]), j]

    factor_list = np.ravel(np.zeros((7,1)))
    clusters = np.zeros((2100,))
    for i in range(len(shape_view)):
        for j in range(c):
            factor_list[j] = 2 * (1 - kernel_gh(hyperparameter_vector, shape_view, prototypes_matrix, i, j))
    ...
    np.savetxt("clusters_q1_shapev.txt", results_objects, fmt="%.0f", delimiter=",")
    np.savetxt("prototypes_q1_shapev.txt", results_prototypes, delimiter=",")
    np.savetxt("hyperparameter_q1_shapev.txt", results_hyperparameter, delimiter=",")
    np.savetxt("ari_q1_shapev.txt", results_ari)
