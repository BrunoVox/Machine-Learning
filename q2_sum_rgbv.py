...
from q2_parzen_rgbv import h_test_rgbv
...
results_ensemble_rgbv = np.zeros((300, 1))
...
for n in range(300):
    ...
    for i in range(len(train_part)):
        if lmatrix[int(train_part[i])] == "GRASS":
            mean_vector_grass += rgb_view[int(train_part[i])]
        elif lmatrix[int(train_part[i])] == "WINDOW":
            mean_vector_window += rgb_view[int(train_part[i])]
        elif lmatrix[int(train_part[i])] == "CEMENT":
            mean_vector_cement += rgb_view[int(train_part[i])]
        elif lmatrix[int(train_part[i])] == "BRICKFACE":
            mean_vector_brickface += rgb_view[int(train_part[i])]
        elif lmatrix[int(train_part[i])] == "SKY":
            mean_vector_sky += rgb_view[int(train_part[i])]
        elif lmatrix[int(train_part[i])] == "FOLIAGE":
            mean_vector_foliage += rgb_view[int(train_part[i])]
        elif lmatrix[int(train_part[i])] == "PATH":
            mean_vector_path += rgb_view[int(train_part[i])]    
    ...
    for i in range(len(train_part)):
        if lmatrix[int(train_part[i])] == "GRASS":
            covar_grass += np.dot(np.transpose(rgb_view[int(train_part[i])] - mean_vector_grass), rgb_view[int(train_part[i])] - mean_vector_grass)
        elif lmatrix[int(train_part[i])] == "WINDOW":
            covar_window += np.dot(np.transpose(rgb_view[int(train_part[i])] - mean_vector_window), rgb_view[int(train_part[i])] - mean_vector_window)
        elif lmatrix[int(train_part[i])] == "CEMENT":
            covar_cement += np.dot(np.transpose(rgb_view[int(train_part[i])] - mean_vector_cement), rgb_view[int(train_part[i])] - mean_vector_cement)
        elif lmatrix[int(train_part[i])] == "BRICKFACE":
            covar_brickface += np.dot(np.transpose(rgb_view[int(train_part[i])] - mean_vector_brickface), rgb_view[int(train_part[i])] - mean_vector_brickface)
        elif lmatrix[int(train_part[i])] == "SKY":
            covar_sky += np.dot(np.transpose(rgb_view[int(train_part[i])] - mean_vector_sky), rgb_view[int(train_part[i])] - mean_vector_sky)
        elif lmatrix[int(train_part[i])] == "FOLIAGE":
            covar_foliage += np.dot(np.transpose(rgb_view[int(train_part[i])] - mean_vector_foliage), rgb_view[int(train_part[i])] - mean_vector_foliage)
        elif lmatrix[int(train_part[i])] == "PATH":
            covar_path += np.dot(np.transpose(rgb_view[int(train_part[i])] - mean_vector_path), rgb_view[int(train_part[i])] - mean_vector_path)
    ...
    for i in range(0, len(test_part)):
        ...
        px_grass = multivar_density(rgb_view[int(test_part[i])], np.ravel(mean_vector_grass), covar_grass)
        px_window = multivar_density(rgb_view[int(test_part[i])], np.ravel(mean_vector_window), covar_window)
        px_cement = multivar_density(rgb_view[int(test_part[i])], np.ravel(mean_vector_cement), covar_cement)
        px_brickface = multivar_density(rgb_view[int(test_part[i])], np.ravel(mean_vector_brickface), covar_brickface)
        px_sky = multivar_density(rgb_view[int(test_part[i])], np.ravel(mean_vector_sky), covar_sky)
        px_foliage = multivar_density(rgb_view[int(test_part[i])], np.ravel(mean_vector_foliage), covar_foliage)
        px_path = multivar_density(rgb_view[int(test_part[i])], np.ravel(mean_vector_path), covar_path)
    ...
    result_ensemble_rgbv = accuracy_score(test_result[:,0], test_result[:,1])
    print("A acurácia da iteração", n+1, "foi de:", result_ensemble_rgbv)

    for i in range(n, n + 1):
        for j in range(1):
            results_ensemble_rgbv[i, j] = result_ensemble_rgbv

    np.savetxt("q2_ensemble_rgbv.txt", results_ensemble_rgbv, fmt="%.6f")