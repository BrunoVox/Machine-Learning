...
from main_parzen import part_matrix, train_part, valid_part, test_part
...
for i in range(0, len(train_part)):
    ...
    if lmatrix[int(train_part[i])] == "WINDOW":
        label_parzen[int(train_part[i])] = 2
    if lmatrix[int(train_part[i])] == "CEMENT":
        label_parzen[int(train_part[i])] = 3
    if lmatrix[int(train_part[i])] == "BRICKFACE":
        label_parzen[int(train_part[i])] = 4
    if lmatrix[int(train_part[i])] == "SKY":
        label_parzen[int(train_part[i])] = 5
    if lmatrix[int(train_part[i])] == "FOLIAGE":
        label_parzen[int(train_part[i])] = 6
    if lmatrix[int(train_part[i])] == "PATH":
...
for i in range(0, len(valid_part)):
    ...
    if lmatrix[int(valid_part[i])] == "WINDOW":
        label_parzen[int(valid_part[i])] = 2
    if lmatrix[int(valid_part[i])] == "CEMENT":
        label_parzen[int(valid_part[i])] = 3
    if lmatrix[int(valid_part[i])] == "BRICKFACE":
        label_parzen[int(valid_part[i])] = 4
    if lmatrix[int(valid_part[i])] == "SKY":
        label_parzen[int(valid_part[i])] = 5
    if lmatrix[int(valid_part[i])] == "FOLIAGE":
        label_parzen[int(valid_part[i])] = 6
    if lmatrix[int(valid_part[i])] == "PATH":
...
for i in range(0, len(test_part)):
    ...
    if lmatrix[int(test_part[i])] == "WINDOW":
        label_parzen[int(test_part[i])] = 2
    if lmatrix[int(test_part[i])] == "CEMENT":
        label_parzen[int(test_part[i])] = 3
    if lmatrix[int(test_part[i])] == "BRICKFACE":
        label_parzen[int(test_part[i])] = 4
    if lmatrix[int(test_part[i])] == "SKY":
        label_parzen[int(test_part[i])] = 5
    if lmatrix[int(test_part[i])] == "FOLIAGE":
        label_parzen[int(test_part[i])] = 6
    if lmatrix[int(test_part[i])] == "PATH":
    ...
    for i in range(0, len(valid_part)):
        ...
        for j in range(0, len(train_part)):
            if label_parzen[int(train_part[j])] == 1:
                ...
                for c in range(0, 18):                            
                    if prod_grass == 0:
                        prod_grass = parzen_window(complete_view[int(valid_part[i]), c], complete_view[int(train_part[j]), c], hx)
                    else:
                        prod_grass *= parzen_window(complete_view[int(valid_part[i]), c], complete_view[int(train_part[j]), c], hx)
                somatorio_grass += prod_grass
            if label_parzen[int(train_part[j])] == 2:
                prod_window = 0
                for c in range(0, 18):                            
                    if prod_window == 0:
                        prod_window = parzen_window(complete_view[int(valid_part[i]), c], complete_view[int(train_part[j]), c], hx)
                    else:
                        prod_window *= parzen_window(complete_view[int(valid_part[i]), c], complete_view[int(train_part[j]), c], hx)
                somatorio_window += prod_window
            if label_parzen[int(train_part[j])] == 3:
                prod_cement = 0
                for c in range(0, 18):                            
                    if prod_cement == 0:
                        prod_cement = parzen_window(complete_view[int(valid_part[i]), c], complete_view[int(train_part[j]), c], hx)
                    else:
                        prod_cement *= parzen_window(complete_view[int(valid_part[i]), c], complete_view[int(train_part[j]), c], hx)
                somatorio_cement += prod_cement
            if label_parzen[int(train_part[j])] == 4:
                prod_brickface = 0
                for c in range(0, 18):                            
                    if prod_brickface == 0:
                        prod_brickface = parzen_window(complete_view[int(valid_part[i]), c], complete_view[int(train_part[j]), c], hx)
                    else:
                        prod_brickface *= parzen_window(complete_view[int(valid_part[i]), c], complete_view[int(train_part[j]), c], hx)
                somatorio_brickface += prod_brickface
            if label_parzen[int(train_part[j])] == 5:
                prod_sky = 0
                for c in range(0, 18):                            
                    if prod_sky == 0:
                        prod_sky = parzen_window(complete_view[int(valid_part[i]), c], complete_view[int(train_part[j]), c], hx)
                    else:
                        prod_sky *= parzen_window(complete_view[int(valid_part[i]), c], complete_view[int(train_part[j]), c], hx)
                somatorio_sky += prod_sky
            if label_parzen[int(train_part[j])] == 6:
                prod_foliage = 0
                for c in range(0, 18):                            
                    if prod_foliage == 0:
                        prod_foliage = parzen_window(complete_view[int(valid_part[i]), c], complete_view[int(train_part[j]), c], hx)
                    else:
                        prod_foliage *= parzen_window(complete_view[int(valid_part[i]), c], complete_view[int(train_part[j]), c], hx)
                somatorio_foliage += prod_foliage
            if label_parzen[int(train_part[j])] == 7:
                prod_path = 0
                for c in range(0, 18):                            
                    if prod_path == 0:
                        prod_path = parzen_window(complete_view[int(valid_part[i]), c], complete_view[int(train_part[j]), c], hx)
                    else:
                        prod_path *= parzen_window(complete_view[int(valid_part[i]), c], complete_view[int(train_part[j]), c], hx)
